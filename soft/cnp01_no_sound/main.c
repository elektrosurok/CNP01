...
void button_act_B()
{
  ...
  case 40:      //Timer mode 1
    ...
    case 0x04:
      ...
      timer_days=ByteToBcd2(timer_temp/84600);
      new_watch_state(41);
      break;
    }
    break;
...
void new_watch_state (uint8_t state)
{
  ...
  case 9:
    ...
    ADC_TempSensorVrefintCmd(ENABLE);// INIT end
    ADC_RegularChannelConfig(ADC1,ADC_Channel_17,1,ADC_SampleTime_384Cycles);
    ADC_SoftwareStartConv(ADC1);
    adc_res=ADC_GetConversionValue(ADC1);
    ADC_DeInit(ADC1); // DEINIT srart
    ADC_Cmd(ADC1, DISABLE); 
    RCC_HSICmd(DISABLE); //DEINIT finish
    adc_res=4096*120/adc_res;
    
    
    lcd_clear();
    lcd_SetChar(1,font[6]);
   ...
   switch(watch_state)
     ...
     case 18:
       ...
       if (RTC_TimeStruct.RTC_Seconds!=current_second)
       {
         ...
         RTC_GetDate(RTC_Format_BCD,&RTC_DateStruct);
         if ((RTC_DateStruct.RTC_Date+temp_date)!=current_day)
         {
           int8_t temp_day=0;
           int8_t temp_month=0;
           uint8_t temp_year=0;
           current_day = RTC_DateStruct.RTC_Date+temp_date;
           if (temp_date)
           {
